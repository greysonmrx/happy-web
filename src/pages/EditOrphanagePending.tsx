...
import '../styles/pages/edit-orphanage-pending.css';
...
interface EditOrphanagePendingParams {
  id: string;
}

const EditOrphanagePending: React.FC = () => {
  const history = useHistory();
  const { id } = useParams<EditOrphanagePendingParams>();
  ...
  async function handleRefuseOrpahange() {
    try {
      await api.delete(`/orphanages/pending/${id}`);

      history.push('/orphanages_pending');
    } catch(err) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      await api.put(`/orphanages/pending/${id}`, {
        name,
        about,
        latitude,
        longitude,
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await api.get<Orphanage>(`/orphanages/pending/${id}`);
  ...
  return (
    <div id="page-edit-orphanage-pending">
      <Sidebar />
          ...
          <form onSubmit={handleSubmit} className="edit-orphanage-form">
            ...
            <div className="buttons-container">
              <button
                ...
                type="button"
                onClick={handleRefuseOrpahange}
              >
                <FiXCircle size={24} color="#FFF" />
                Recusar
              </button>
              <button className="button confirm-button" type="submit">
...
export default EditOrphanagePending;
